if (!global.db) {
  ...
  function getroomusers(room_id) {
    const sql = `
          SELECT users.id, users.name AS user_name, users.email, users.room_id, users.password, users.color, users.reminder, users.state,users.expect, users.photo, rooms.name AS room_name
          FROM users
          INNER JOIN rooms
